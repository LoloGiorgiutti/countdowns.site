...
(function(){{
  ...
  document.getElementById('cd-weekday-section').innerHTML=
    ...
    if ev_type == "annual" and ev.get("_month") and ev.get("_day"):
        ...
        if _births:
            _li = "".join(
                f'<li><span class="cd-birth-name">{b["name"]}</span><span class="cd-birth-desc">{translate_known_for(b["known_for"], lang)}</span></li>'
                for b in _births
            )
            births_section = f'<div class="cd-births"><h3 class="cd-births-title">{_bt}</h3><ul class="cd-births-list">{_li}</ul></div>'
        if _events:
            _li = "".join(
                f'<li><span class="cd-hist-year">{e["year"]}</span><span class="cd-hist-desc">{e["event"]}</span></li>'
                for e in _events
            )
...
{'''.cd-weekday-table{margin:1.5rem 0}.cd-wt-title{font-size:.95rem;font-weight:600;margin-bottom:.6rem;opacity:.9}.cd-wt{border-collapse:collapse;width:100%;max-width:280px}.cd-wt td{padding:.35rem .7rem;font-size:.9rem}.cd-wt-yr{opacity:.6}.cd-wt-target{font-weight:700}.cd-wt-target .cd-wt-yr{opacity:1;color:var(--cat-color)}.cd-wt-target .cd-wt-day{color:var(--cat-color)}.cd-births,.cd-hist{margin:1.5rem 0;background:rgba(255,255,255,.04);border-radius:12px;padding:.9rem 1.1rem;border:1px solid rgba(255,255,255,.07)}.cd-births-title,.cd-hist-title{font-size:.8rem;font-weight:700;margin:0 0 .75rem;letter-spacing:.06em;text-transform:uppercase;color:var(--cat-color)}.cd-births-list,.cd-hist-list{list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:.5rem}.cd-births-list li,.cd-hist-list li{font-size:.88rem;line-height:1.4;padding:.5rem .65rem;background:rgba(255,255,255,.03);border-radius:8px;border-left:3px solid var(--cat-color)}.cd-birth-name{font-weight:700;display:block;margin-bottom:.1rem}.cd-birth-desc{opacity:.75;font-size:.85rem}.cd-hist-year{font-weight:700;color:var(--cat-color);margin-right:.4rem}.cd-hist-desc{opacity:.85}''' if ev_type=='annual' else ''}</style>
...
# ─── Phrase-level translation for birthday known_for descriptions ──────────────
# Each language lists (English phrase, translated phrase) pairs.
# Applied as prefix-match first, then anywhere in the string.
_KF_TRANS = {
  'es': [
    ("Three-time Oscar-winning actor","Actor ganador de tres Óscares"),
    ("Two-time Oscar-winning actor","Actor ganador de dos Óscares"),
    ("Three-time Oscar-winning actress","Actriz ganadora de tres Óscares"),
    ("Two-time Oscar-winning actress","Actriz ganadora de dos Óscares"),
    ("EGOT winner","Ganador/a del EGOT (Emmy, Grammy, Óscar y Tony)"),
    ("Oscar-winning actor","Actor ganador del Óscar"),
    ("Oscar-winning actress","Actriz ganadora del Óscar"),
    ("Grammy-winning singer","Cantante ganadora del Grammy"),
    ("Grammy-winning","Premiado con el Grammy"),
    ("Nobel Peace Prize winner","Ganador/a del Premio Nobel de la Paz"),
    ("Nobel Prize-winning","Ganador del Premio Nobel"),
    ("Civil rights leader","Líder de los derechos civiles"),
    ("Civil rights activist","Activista por los derechos civiles"),
    ("First female","Primera mujer"),
    ("First Black","Primer/a persona negra en"),
    ("First human to walk on the Moon","Primer humano en caminar sobre la Luna"),
    ("Lead singer and guitarist","Vocalista y guitarrista"),
    ("Lead singer","Vocalista"),
    ("Founder and lead singer","Fundador y vocalista"),
    ("Singer-songwriter and pianist","Cantautor/a y pianista"),
    ("Singer-songwriter","Cantautor/a"),
    ("Singer and actress","Cantante y actriz"),
    ("Singer and actor","Cantante y actor"),
    ("Rock and soul legend","Leyenda del rock y el soul"),
    ("Rock and soul","Rock y soul"),
    ("Rock musician","Músico de rock"),
    ("Rock singer","Cantante de rock"),
    ("R&B and soul singer","Cantante de R&B y soul"),
    ("R&B and soul icon","Ícono del R&B y el soul"),
    ("Pop superstar","Superestrella del pop"),
    ("Pop and R&B singer","Cantante de pop y R&B"),
    ("Pop legend","Leyenda del pop"),
    ("Pop star","Estrella del pop"),
    ("Pop icon","Ícono del pop"),
    ("Pop and hip-hop artist","Artista de pop y hip-hop"),
    ("Rapper and actor","Rapero y actor"),
    ("Rapper","Rapero"),
    ("Comedian and actor","Comediante y actor"),
    ("Comedian","Comediante"),
    ("R&B and soul icon","Ícono del R&B y el soul"),
    ("R&B singer","Cantante de R&B"),
    ("Pop singer","Cantante de pop"),
    ("Singer","Cantante"),
    ("TV host","Presentador/a de televisión"),
    ("Radio and TV personality","Personalidad de la radio y televisión"),
    ("Actor and director","Actor y director"),
    ("Actor and filmmaker","Actor y cineasta"),
    ("Actor and comedian","Actor y comediante"),
    ("Actor and producer","Actor y productor"),
    ("Actor","Actor"),
    ("Actress and activist","Actriz y activista"),
    ("Actress and singer","Actriz y cantante"),
    ("Actress and model","Actriz y modelo"),
    ("Actress","Actriz"),
    ("Director","Director"),
    ("Filmmaker","Cineasta"),
    ("Author of","Autor de"),
    ...
    ("Model","Modelo"),
    ("Celebrity chef","Chef famoso"),
    ("Astronaut","Astronauta"),
    ("President of","Presidente de"),
    ...
    ("football star","estrella del fútbol"),
    ("footballer","futbolista"),
    ("boxing legend","leyenda del boxeo"),
    ("boxing champion","campeón de boxeo"),
    ("tennis legend","leyenda del tenis"),
    ("tennis champion","campeón/a de tenis"),
    ...
    ("Canadian","Canadiense"),
    ("one of the greatest singers of all time","una de las mejores cantantes de todos los tiempos"),
    ("best-selling","más vendido"),
    ("world's fastest man","el hombre más rápido del mundo"),
    ("who developed the theory of relativity","que desarrolló la teoría de la relatividad"),
    ("who proposed the theory of evolution","que propuso la teoría de la evolución"),
    ("The King of Rock and Roll","El Rey del Rock and Roll"),
    ("Queen of Pop","Reina del Pop"),
    ("Queen of Soul","Reina del Soul"),
    ("King of Pop","Rey del Pop"),
    ("The Boss","El Jefe"),
    ("Godfather of Soul","El Padrino del Soul"),
    ("Legendary rock musician","Legendario músico de rock"),
    ("Legendary British musician","Legendario músico británico"),
    ("Theoretical physicist","Físico teórico"),
    ("and cultural icon","e ícono cultural"),
    ("cultural icon","ícono cultural"),
    ("who developed","que desarrolló"),
    ("who formulated","que formuló"),
    ("who invented","que inventó"),
    ("who proposed","que propuso"),
    ("who discovered","que descubrió"),
    ("who championed","que defendió"),
    ("three-time world heavyweight champion","tricampeón mundial de peso pesado"),
    ("Former First Lady","Ex Primera Dama"),
    ("former First Lady","ex Primera Dama"),
    ("Founding Father","Padre Fundador"),
    ("undefeated","invicto"),
    ("world heavyweight","peso pesado mundial"),
    ("multiple","múltiples"),
    ("time Grammy winner","veces ganador del Grammy"),
    ("time Olympic gold","veces medalla de oro olímpica"),
    ("Action actor","Actor de acción"),
    ("and director","y director"),
    ("and filmmaker","y cineasta"),
    ("and actress","y actriz"),
    ("and producer","y productor"),
  ...
  'pt': [
    ("Three-time Oscar-winning actor","Ator vencedor de três Oscares"),
    ("Two-time Oscar-winning actor","Ator vencedor de dois Oscares"),
    ("Three-time Oscar-winning actress","Atriz vencedora de três Oscares"),
    ("Two-time Oscar-winning actress","Atriz vencedora de dois Oscares"),
    ("EGOT winner","Vencedor/a do EGOT (Emmy, Grammy, Oscar e Tony)"),
    ("Oscar-winning actor","Ator vencedor do Oscar"),
    ("Oscar-winning actress","Atriz vencedora do Oscar"),
    ("Grammy-winning singer","Cantora vencedora do Grammy"),
    ("Grammy-winning","Premiado/a com o Grammy"),
    ("Nobel Peace Prize winner","Vencedor/a do Prémio Nobel da Paz"),
    ("Nobel Prize-winning","Vencedor do Prémio Nobel"),
    ("Civil rights leader","Líder dos direitos civis"),
    ("Civil rights activist","Ativista pelos direitos civis"),
    ("First female","Primeira mulher"),
    ("First Black","Primeiro/a negro/a a"),
    ("First human to walk on the Moon","Primeiro humano a caminhar na Lua"),
    ("Lead singer and guitarist","Vocalista e guitarrista"),
    ("Lead singer","Vocalista"),
    ("Founder and lead singer","Fundador e vocalista"),
    ("Singer-songwriter and pianist","Cantor/a-compositor/a e pianista"),
    ("Singer-songwriter","Cantor/a-compositor/a"),
    ("Singer and actress","Cantora e atriz"),
    ("Singer and actor","Cantor e ator"),
    ("Rock and soul legend","Lenda do rock e soul"),
    ("Rock and soul","Rock e soul"),
    ("Rock musician","Músico de rock"),
    ("Rock singer","Cantor/a de rock"),
    ("R&B and soul singer","Cantor/a de R&B e soul"),
    ("R&B and soul icon","Ícone do R&B e soul"),
    ("Pop superstar","Superestrela do pop"),
    ("Pop and R&B singer","Cantor/a de pop e R&B"),
    ("Pop legend","Lenda do pop"),
    ("Pop star","Estrela do pop"),
    ("Pop icon","Ícone do pop"),
    ("Pop and hip-hop artist","Artista de pop e hip-hop"),
    ("Rapper and actor","Rapper e ator"),
    ("Rapper","Rapper"),
    ("Comedian and actor","Comediante e ator"),
    ("Comedian","Comediante"),
    ("R&B and soul icon","Ícone do R&B e soul"),
    ("R&B singer","Cantor/a de R&B"),
    ("Pop singer","Cantor/a de pop"),
    ("Singer","Cantor/a"),
    ("TV host","Apresentador/a de televisão"),
    ("Radio and TV personality","Personalidade do rádio e televisão"),
    ("Actor and director","Ator e diretor"),
    ("Actor and filmmaker","Ator e cineasta"),
    ("Actor and comedian","Ator e comediante"),
    ("Actor and producer","Ator e produtor"),
    ("Actor","Ator"),
    ("Actress and activist","Atriz e ativista"),
    ("Actress and singer","Atriz e cantora"),
    ("Actress and model","Atriz e modelo"),
    ("Actress","Atriz"),
    ("Director","Diretor"),
    ("Filmmaker","Cineasta"),
    ("Author of","Autor de"),
    ...
    ("football legend","lenda do futebol"),
    ("football star","estrela do futebol"),
    ("footballer","futebolista"),
    ("boxing legend","lenda do boxe"),
    ("boxing champion","campeão/ã de boxe"),
    ("tennis legend","lenda do tênis"),
    ("tennis champion","campeão/ã de tênis"),
    ("Legendary","Lendário"),
    ("legend","lenda"),
    ("icon","ícone"),
    ("pioneer","pioneiro"),
    ...
    ("Australian","Australiano"),
    ("Canadian","Canadense"),
    ("one of the greatest singers of all time","uma das maiores cantoras de todos os tempos"),
    ("best-selling","mais vendido"),
    ("world's fastest man","o homem mais rápido do mundo"),
    ("who developed the theory of relativity","que desenvolveu a teoria da relatividade"),
    ("who proposed the theory of evolution","que propôs a teoria da evolução"),
    ("The King of Rock and Roll","O Rei do Rock and Roll"),
    ("Queen of Pop","Rainha do Pop"),
    ("Queen of Soul","Rainha do Soul"),
    ("King of Pop","Rei do Pop"),
    ("Godfather of Soul","O Padrinho do Soul"),
    ("Legendary rock musician","Lendário músico de rock"),
    ("Legendary British musician","Lendário músico britânico"),
    ("Theoretical physicist","Físico teórico"),
    ("and cultural icon","e ícone cultural"),
    ("cultural icon","ícone cultural"),
    ("who developed","que desenvolveu"),
    ("who formulated","que formulou"),
    ("who invented","que inventou"),
    ("who proposed","que propôs"),
    ("who discovered","que descobriu"),
    ("who championed","que defendeu"),
    ("three-time world heavyweight champion","tricampeão mundial dos pesos pesados"),
    ("Former First Lady","Ex-Primeira Dama"),
    ("former First Lady","ex-Primeira Dama"),
    ("Founding Father","Pai Fundador"),
    ("undefeated","invicto"),
    ("world heavyweight","peso pesado mundial"),
    ("multiple","múltiplos"),
    ("Action actor","Ator de ação"),
    ("and director","e diretor"),
    ("and filmmaker","e cineasta"),
    ("and actress","e atriz"),
    ("and producer","e produtor"),
    ("of the United States","dos Estados Unidos"),
  ...
  'fr': [
    ("Three-time Oscar-winning actor","Acteur trois fois oscarisé"),
    ("Two-time Oscar-winning actor","Acteur deux fois oscarisé"),
    ("Three-time Oscar-winning actress","Actrice trois fois oscarisée"),
    ("Two-time Oscar-winning actress","Actrice deux fois oscarisée"),
    ("EGOT winner","Lauréat/e de l'EGOT (Emmy, Grammy, Oscar et Tony)"),
    ("Oscar-winning actor","Acteur oscarisé"),
    ("Oscar-winning actress","Actrice oscarisée"),
    ("Grammy-winning singer","Chanteuse lauréate d'un Grammy"),
    ("Grammy-winning","Lauréat/e d'un Grammy"),
    ("Nobel Peace Prize winner","Lauréat/e du Prix Nobel de la Paix"),
    ("Nobel Prize-winning","Lauréat du Prix Nobel"),
    ("Civil rights leader","Leader des droits civiques"),
    ("Civil rights activist","Militant des droits civiques"),
    ("First female","Première femme"),
    ("First Black","Premier/ère Noir/e à"),
    ("First human to walk on the Moon","Premier humain à marcher sur la Lune"),
    ("Lead singer and guitarist","Chanteur principal et guitariste"),
    ("Lead singer","Chanteur/euse principal/e"),
    ("Founder and lead singer","Fondateur et chanteur principal"),
    ("Singer-songwriter and pianist","Auteur-compositeur-interprète et pianiste"),
    ("Singer-songwriter","Auteur-compositeur-interprète"),
    ("Singer and actress","Chanteuse et actrice"),
    ("Singer and actor","Chanteur et acteur"),
    ("Rock and soul legend","Légende du rock et soul"),
    ("Rock and soul","Rock et soul"),
    ("Rock musician","Musicien de rock"),
    ("Rock singer","Chanteur/euse de rock"),
    ("R&B and soul singer","Chanteur/euse de R&B et soul"),
    ("R&B and soul icon","Icône du R&B et soul"),
    ("Pop superstar","Superstar de la pop"),
    ("Pop and R&B singer","Chanteur/euse de pop et R&B"),
    ("Pop legend","Légende de la pop"),
    ("Pop star","Star de la pop"),
    ("Pop icon","Icône de la pop"),
    ("Pop and hip-hop artist","Artiste de pop et hip-hop"),
    ("Rapper and actor","Rappeur et acteur"),
    ("Rapper","Rappeur"),
    ("Comedian and actor","Comédien et acteur"),
    ("Comedian","Comédien"),
    ("R&B and soul icon","Icône du R&B et soul"),
    ("R&B singer","Chanteur/euse de R&B"),
    ("Pop singer","Chanteur/euse de pop"),
    ("Singer","Chanteur/euse"),
    ("TV host","Présentateur/trice de télévision"),
    ("Radio and TV personality","Personnalité de la radio et de la télévision"),
    ("Actor and director","Acteur et réalisateur"),
    ("Actor and filmmaker","Acteur et cinéaste"),
    ("Actor and comedian","Acteur et comédien"),
    ("Actor and producer","Acteur et producteur"),
    ("Actor","Acteur"),
    ("Actress and activist","Actrice et militante"),
    ("Actress and singer","Actrice et chanteuse"),
    ("Actress and model","Actrice et mannequin"),
    ("Actress","Actrice"),
    ...
    ("Model","Mannequin"),
    ("Celebrity chef","Chef cuisinier célèbre"),
    ("Astronaut","Astronaute"),
    ("President of","Président de"),
    ("President","Président"),
    ("activist","militant"),
    ("scientist","scientifique"),
    ("journalist","journaliste"),
    ("fashion designer","créateur/trice de mode"),
    ("philanthropist","philanthrope"),
    ("football legend","légende du football"),
    ("football star","star du football"),
    ("footballer","footballeur"),
    ("boxing legend","légende de la boxe"),
    ("boxing champion","champion de boxe"),
    ("tennis legend","légende du tennis"),
    ("tennis champion","champion/ne de tennis"),
    ...
    ("Canadian","Canadien"),
    ("one of the greatest singers of all time","l'une des plus grandes chanteuses de tous les temps"),
    ("best-selling","le plus vendu"),
    ("world's fastest man","l'homme le plus rapide du monde"),
    ("who developed the theory of relativity","qui a développé la théorie de la relativité"),
    ("who proposed the theory of evolution","qui a proposé la théorie de l'évolution"),
    ("The King of Rock and Roll","Le Roi du Rock and Roll"),
    ("Queen of Pop","Reine de la Pop"),
    ("Queen of Soul","Reine de la Soul"),
    ("King of Pop","Roi de la Pop"),
    ("Godfather of Soul","Le Parrain du Soul"),
    ("Legendary rock musician","Légendaire musicien de rock"),
    ("Legendary British musician","Légendaire musicien britannique"),
    ("Theoretical physicist","Physicien théoricien"),
    ("and cultural icon","et icône culturelle"),
    ("cultural icon","icône culturelle"),
    ("who developed","qui a développé"),
    ("who formulated","qui a formulé"),
    ("who invented","qui a inventé"),
    ("who proposed","qui a proposé"),
    ("who discovered","qui a découvert"),
    ("who championed","qui a défendu"),
    ("three-time world heavyweight champion","triple champion du monde poids lourd"),
    ("Former First Lady","Ancienne Première Dame"),
    ("former First Lady","ancienne Première Dame"),
    ("Founding Father","Père fondateur"),
    ("undefeated","invaincu"),
    ("world heavyweight","poids lourd mondial"),
    ("multiple","plusieurs"),
    ("Action actor","Acteur d'action"),
    ("and director","et réalisateur"),
    ("and filmmaker","et cinéaste"),
    ("and actress","et actrice"),
    ("and producer","et producteur"),
...
def translate_known_for(text, lang):
  """Translate a known_for description using phrase-level substitution."""
  if lang == 'en' or not text:
    return text
  # Sort longest phrases first so specific matches beat shorter ones
  pairs = sorted(_KF_TRANS.get(lang, []), key=lambda x: len(x[0]), reverse=True)
  result = text
  translated_spans = []  # Track (start, end) of already-translated regions

  def _in_translated(idx):
    """Return True if idx falls inside a previously-translated span."""
    for s, e in translated_spans:
      if s <= idx < e:
        return True
    return False

  # Try prefix match first
  for en_phrase, translated in pairs:
    if result.lower().startswith(en_phrase.lower()):
      result = translated + result[len(en_phrase):]
      translated_spans.append((0, len(translated)))
      break

  # Apply substring replacements — but skip any match inside already-translated text
  for en_phrase, translated in pairs:
    lower_phrase = en_phrase.lower()
    search_pos = 0
    while True:
      idx = result.lower().find(lower_phrase, search_pos)
      if idx == -1:
        break
      if _in_translated(idx):
        search_pos = idx + 1
        continue
      # Apply replacement
      result = result[:idx] + translated + result[idx + len(en_phrase):]
      translated_spans.append((idx, idx + len(translated)))
...
def inject_daily_translations(slug, month, day):
  ...
  for lang in ['es','pt','fr']:
    ...
    if _births:
      details_str = " ".join(f"{b['name']} ({translate_known_for(b['known_for'], lang)})." for b in _births)
      if lang == 'fr':
